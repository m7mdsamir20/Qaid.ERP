...
function updatePage() {
    ...
    // 5. Replace UI toggle and PlanType logic
    const uiToggleStart = code.indexOf(`<div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>`);
    const uiToggleEnd = code.indexOf(`{planType === 'direct' ? (`);
    
    // Find the end of the `planType` conditional block (where `<div>\n <label ...>قيمة المنتج الإجمالية` starts)
    const productTotalLabelStart = code.indexOf(`<div>
                                <label style={{ ...LS, fontSize: '11.5px' }}>{t('قيمة المنتج الإجمالية')}`);
    
    if (uiToggleStart !== -1 && productTotalLabelStart !== -1) {
        const cartUI = `
                            {/* Cart Section */}
                            ...
                            `;
        code = code.substring(0, uiToggleStart) + cartUI + code.substring(productTotalLabelStart);
    }

    // 6. Make Total Amount disabled and remove planType ref
...
updatePage();
console.log('UI completely updated');
