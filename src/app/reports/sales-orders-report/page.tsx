...
import { Receipt, Search, Calendar, Loader2, TrendingUp, FileText, CheckCircle, Clock } from 'lucide-react';
...
export default function SalesOrdersReportPage() {
    ...
    const [stats, setStats] = useState({ totalAmount: 0, count: 0, fullyDeliveredCount: 0, partiallyDeliveredCount: 0 });
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/reports/sales-orders-report?${params}`);
            if (res.ok) {
                ...
                setStats({
                    ...
                    count: json.count || 0,
                    fullyDeliveredCount: json.fullyDeliveredCount || 0,
    ...
    const columns: TableColumn[] = [
        {
            ...
            cell: (row: SalesOrder) => (
                <span style={{ background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.2)', borderRadius: '8px', padding: '3px 10px', fontSize: '11.5px', fontWeight: 600, color: '#22c55e', fontFamily: OUTFIT }}>
        ...
        {
            header: t('حالة التسليم'),
            cell: (row: SalesOrder) => {
                if (!row.lines || row.lines.length === 0) return t('غير مسلم');
                ...
                if (totalDelivered >= totalOrdered && totalOrdered > 0) {
                    return <span style={{ color: '#10b981', fontWeight: 600, fontFamily: CAIRO }}>{t('مسلم بالكامل')}</span>;
                }
                if (totalDelivered > 0) {
                    return <span style={{ color: '#fb923c', fontWeight: 600, fontFamily: CAIRO }}>{`${t('مسلم جزئياً')} (${Math.round((totalDelivered / totalOrdered) * 100)}%)`}</span>;
                }
                return <span style={{ color: '#ef4444', fontWeight: 600, fontFamily: CAIRO }}>{t('غير مسلم')}</span>;
            },
    ...
    return (
        <DashboardLayout>
            <div dir={isRtl ? 'rtl' : 'ltr'} style={PAGE_BASE}>
                <ReportHeader
                    title={t("تقرير أوامر البيع")}
                    subtitle={t("قائمة أوامر البيع مع حالة التسليم والفوترة والمجاميع الكلية.")}
                    backTab="sales-purchases"
                    printTitle={t("تقرير أوامر البيع تفصيلي")}
                ...
                <div data-print-stats style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px', marginBottom: '24px' }}>
                    ...
                    />
                    <StatCard
                        label={t('عدد أوامر البيع')}
                        value={String(stats.count)}
                        icon={<FileText size={18} />}
                        color="#a78bfa"
                    />
                    <StatCard
                        label={t('أوامر مسلمة بالكامل')}
                        value={String(stats.fullyDeliveredCount)}
                        icon={<CheckCircle size={18} />}
                        color="#10b981"
                    />
                    <StatCard
                        label={t('أوامر مسلمة جزئياً')}
                        value={String(stats.partiallyDeliveredCount)}
                        icon={<Clock size={18} />}
                        color="#fb923c"
                    />
                </div>
                    ...
                    {loading ? (
                        ...
                        <div className="print-table-container">
                            <DataTable
                                ...
                                data={filteredOrders}
                                emptyIcon={Receipt}
                                emptyMessage={t('لا توجد أوامر بيع مسجلة للفترة المحددة')}
                            />
                        </div>
                    )}
                </div>
