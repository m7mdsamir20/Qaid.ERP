...
import {
    Calendar, Search, Printer, Wallet,
...
export default function CashStatementPage() {
        ...
        const fetchTreasuries = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    setTreasuries(Array.isArray(all) ? all.filter((t: any) => t.type === 'cash') : []);
                }
            } catch { }
        };
        fetchTreasuries();
    ...
    const movements = (data && Array.isArray(data.movements)) ? (() => {
                ...
                <div className="print-only">
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', paddingBottom: '12px', borderBottom: '2px solid #000' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <h3 style={{ margin: '0 0 6px', fontSize: '14px', fontWeight: 900, color: '#000', fontFamily: CAIRO }}>كشف حركة الخزينة</h3>
                    ...
                    <>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '24px' }}>
                            {[
                                ...
                                { label: 'إجمالي المقبوضات', value: data.movements.reduce((s: number, m: any) => m.type === 'receipt' ? s + m.amount : s, 0), color: SC, icon: <TrendingUp size={20} />, sign: 'وارد للخزينة (+)' },
                                { label: 'إجمالي المدفوعات', value: data.movements.reduce((s: number, m: any) => m.type === 'payment' ? s + m.amount : s, 0), color: DC, icon: <TrendingDown size={20} />, sign: 'صادر من الخزينة (-)' },
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    ...
                                    <tbody>
                                        <tr style={{ background: 'rgba(255,255,255,0.01)', borderBottom: `1px solid ${C.border}` }}>
                                            <td colSpan={2} style={{ padding: '14px 20px', textAlign: 'right', fontSize: '12.5px', color: C.textPrimary, fontWeight: 900, fontFamily: CAIRO }}>رصيد افتتاحي (قبل الفترة المحددة)</td>
