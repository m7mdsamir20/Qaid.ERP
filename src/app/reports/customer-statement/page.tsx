...
export default function CustomerStatementPage() {
    ...
    const fetchLedger = useCallback(async (customerId: string = selectedId) => {
        ...
        } catch { setError('خطا في الاتصال بالخادم'); } finally { setLoading(false); }
    ...
    const exportToExcel = () => {
        ...
        if (data.initialBalance !== 0) {
            excelData.unshift({
                'التاريخ': data.customer?.createdAt ? new Date(data.customer.createdAt).toLocaleDateString('en-GB') : '—',
                'طبيعة الحركة': 'رصيد افتتاحي',
                'المرجع': '—',
                'البيان': 'رصيد ما قبل فترة التقرير',
                'مدين (عليه)': data.initialBalance > 0 ? data.initialBalance : 0,
                'دائن (له)': data.initialBalance < 0 ? Math.abs(data.initialBalance) : 0,
        ...
        XLSX.writeFile(wb, `كشف_حساب_${data.customer.name}_${new Date().toLocaleDateString('en-GB')}.xlsx`);
    };
            ...
            <div dir="rtl" style={PAGE_BASE}>
                ...
                <div className="print-only">
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', paddingBottom: '12px', borderBottom: '2px solid #000' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <h3 style={{ margin: '0 0 6px', fontSize: '14px', fontWeight: 900, color: '#000', fontFamily: CAIRO }}>كشف حساب عميل تفصيلي</h3>
                            {data && <div style={{ fontSize: '12px', color: '#000', fontWeight: 800, fontFamily: CAIRO }}>{data.customer.name}</div>}
                        </div>
                ...
                <div className="no-print" style={{ display: 'flex', gap: '14px', marginBottom: '24px', alignItems: 'center', width: '100%', padding: 0 }}>
                    <div style={{ flex: 2, position: 'relative' }}>
                        <CustomSelect
                            ...
                            placeholder="اختر العميل لمتابعة حسابه..."
                            options={[
                                { value: '', label: '-- اختر عميلاً من القائمة --' },
                ...
                {loading ? (
                    <div style={{ padding: '100px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', gap: '16px' }}>
                        ...
                        <span style={{ fontWeight: 700, fontFamily: CAIRO, color: C.textSecondary }}>جاري استخراج السجلات...</span>
                    </div>
                ) : !data ? (
                    <div style={{ padding: '120px', textAlign: 'center', background: C.card, border: `1px solid ${C.border}`, borderRadius: '24px' }}>
                        ...
                        <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: C.textPrimary, fontFamily: CAIRO }}>بانتظار اختيار العميل</h3>
                        <p style={{ margin: '10px 0 0', fontSize: '12.5px', color: C.textMuted, fontFamily: CAIRO }}>يرجى اختيار العميل وتحديد الفترة الزمنية لعرض كشف الحساب التفصيلي.</p>
                    </div>
                ) : (
                    <>
                        {/* Summary Stats Cards */}
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    <thead>
                                        <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: `1px solid ${C.border}` }}>
                                            {['التاريخ', 'طبيعة الحركة', 'المـرجع', 'البيان والتفاصيل', 'عليه (+)', 'لـه (-)', 'الرصيد'].map((h, i) => (
                                                <th key={i} style={{ 
