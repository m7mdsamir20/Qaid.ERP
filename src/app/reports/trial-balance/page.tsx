                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead>
                                    ...
                                    <tr style={{ background: 'rgba(255,255,255,0.01)', borderBottom: `1px solid ${C.border}` }}>
                                        <th style={{ padding: '10px', textAlign: 'center', fontSize: '12px', fontWeight: 800, color: C.textMuted, fontFamily: CAIRO, borderInlineStart: `1px solid ${C.border}` }}>{t('مدين')}</th>
                                        <th style={{ padding: '10px', textAlign: 'center', fontSize: '12px', fontWeight: 800, color: C.textMuted, fontFamily: CAIRO, borderInlineStart: `1px solid ${C.border}` }}>{t('دائن')}</th>
                                        <th style={{ padding: '10px', textAlign: 'center', fontSize: '12px', fontWeight: 800, color: C.textMuted, fontFamily: CAIRO, borderInlineStart: `1px solid ${C.border}` }}>{t('مدين')}</th>
                                        <th style={{ padding: '10px', textAlign: 'center', fontSize: '12px', fontWeight: 800, color: C.textMuted, fontFamily: CAIRO }}>{t('دائن')}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {report.map((line) => (
                                        <tr key={line.code} style={{ borderBottom: `1px solid ${C.border}`, transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.01)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                                            ...
                                            <td style={{ padding: '14px 16px', textAlign: 'center', fontSize: '14px', color: line.totalDebit > 0 ? C.textPrimary : C.textMuted, fontFamily: INTER, borderInlineStart: `1px solid ${C.border}` }}>{line.totalDebit > 0 ? fmt(line.totalDebit) : '0.00'} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                            <td style={{ padding: '14px 16px', textAlign: 'center', fontSize: '14px', color: line.totalCredit > 0 ? C.textPrimary : C.textMuted, fontFamily: INTER, borderInlineStart: `1px solid ${C.border}` }}>{line.totalCredit > 0 ? fmt(line.totalCredit) : '0.00'} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                            <td style={{ padding: '14px 16px', textAlign: 'center', fontSize: '14px', fontWeight: 600, color: line.balanceDebit > 0 ? C.success : C.textMuted, background: 'rgba(16, 185, 129, 0.01)', fontFamily: INTER, borderInlineStart: `1px solid ${C.border}` }}>{line.balanceDebit > 0 ? fmt(line.balanceDebit) : '0.00'} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                            <td style={{ padding: '14px 16px', textAlign: 'center', fontSize: '14px', fontWeight: 600, color: line.balanceCredit > 0 ? C.danger : C.textMuted, background: 'rgba(239, 68, 68, 0.01)', fontFamily: INTER }}>{line.balanceCredit > 0 ? fmt(line.balanceCredit) : '0.00'} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot style={{ background: 'rgba(59, 130, 246, 0.05)', borderTop: `2px solid ${C.primary}44` }}>
                                    <tr>
                                        ...
                                        <td style={{ padding: '18px 16px', textAlign: 'center', fontWeight: 900, color: C.textPrimary, fontSize: '14px', fontFamily: INTER, borderInlineStart: `1px solid ${C.border}` }}>{fmt(grandTotalDebit)} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                        <td style={{ padding: '18px 16px', textAlign: 'center', fontWeight: 900, color: C.textPrimary, fontSize: '14px', fontFamily: INTER, borderInlineStart: `1px solid ${C.border}` }}>{fmt(grandTotalCredit)} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                        <td style={{ padding: '18px 16px', textAlign: 'center', fontWeight: 950, color: C.success, fontSize: '14px', background: 'rgba(16, 185, 129, 0.05)', fontFamily: INTER, borderInlineStart: `1px solid ${C.border}` }}>{fmt(grandBalanceDebit)} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                        <td style={{ padding: '18px 16px', textAlign: 'center', fontWeight: 950, color: C.danger, fontSize: '14px', background: 'rgba(239, 68, 68, 0.05)', fontFamily: INTER }}>{fmt(grandBalanceCredit)} <span style={{ fontFamily: CAIRO, fontSize: '11px', color: C.textMuted, marginInlineStart: '2px' }}>{sym}</span></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
