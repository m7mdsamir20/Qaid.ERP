...
export default function SalesRepresentativesReportPage() {
    ...
    // Auto-fetch whenever the filters change
    useEffect(() => {
        fetchReport();
    }, [from, to, selectedRepId, calculationBasis]);
            ...
            cell: (row: RepresentativeReport) => (
                <div style={{ display: 'flex', alignItems: 'center', gap: '9px', cursor: 'pointer' }} onClick={() => setActiveRepId(row.id)}>
                    <div style={{
                        ...
                        background: activeRepId === row.id ? C.primary : `${C.primary}20`,
    ...
    return (
        <DashboardLayout>
            <div dir={isRtl ? 'rtl' : 'ltr'} style={PAGE_BASE}>
                <ReportHeader
                    title={t("تقرير العمولات")}
                    subtitle={t("احتساب وعرض عمولات المناديب حسب المبيعات أو التحصيل مع إمكانية الاعتماد والصرف.")}
                    backTab="sales_reps"
