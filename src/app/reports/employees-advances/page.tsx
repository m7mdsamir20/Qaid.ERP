        ...
        <DashboardLayout>
            <div dir="rtl" style={PAGE_BASE}>
                ...
                <div className="no-print" style={{ position: 'relative', marginBottom: '20px' }}>
                    ...
                    <input placeholder="ابحث باسم الموظف..." value={q} onChange={e => setQ(e.target.value)} style={{ ...IS, paddingRight: '45px', height: '42px', background: C.card, borderRadius: '12px', border: `1px solid ${C.border}` }} />
                </div>
                                ...
                                {data?.records.filter(r => r.employeeName.includes(q)).map((r, idx) => {
                                    ...
                                    return (
                                        <tr key={r.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                                            ...
                                            <td style={{ padding: '14px 20px', textAlign: 'center', fontWeight: 800, fontFamily: INTER }}>{fmt(r.totalAmount)}</td>
                                            <td style={{ padding: '14px 20px', textAlign: 'center', fontWeight: 800, color: '#10b981', fontFamily: INTER }}>{fmt(r.paidAmount)}</td>
                                            <td style={{ padding: '14px 20px', textAlign: 'center', fontWeight: 800, color: '#ef4444', fontFamily: INTER }}>{fmt(r.remainingAmount)}</td>
