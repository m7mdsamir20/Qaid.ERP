                    ...
                    <>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '24px' }}>
                            {[
                                ...
                                { label: 'المبالغ المسددة', value: fmt(data.totalPaid), color: '#10b981', icon: <ArrowUpRight size={18} /> },
                                { label: 'الأرصدة المستحقة', value: fmt(data.totalRemaining), color: data.totalRemaining > 0 ? '#fb7185' : '#10b981', icon: <DollarSign size={18} /> },
                            ].map((s, i) => (
                                <div key={i} style={{
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: `1px solid ${C.border}` }}>
                                        {['رقم الفاتورة', 'التاريخ', 'اسم المورد', 'إجمالي القيمة', 'الخصم', 'المسدد', 'المتبقي'].map((h, i) => (
                                            <th key={i} style={{ 
                                                padding: '16px 20px', 
                                                fontSize: '12px', 
                                                fontWeight: 800, 
                                ...
                                <tfoot style={{ background: 'rgba(255,255,255,0.03)', borderTop: `2px solid ${C.border}` }}>
                                    <tr>
                                        <td colSpan={3} style={{ padding: '18px 24px', fontSize: '13px', fontWeight: 900, color: C.textSecondary, fontFamily: CAIRO }}>إجماليات المشتريات للفترة</td>
