    ...
    if (loading) return (
        <DashboardLayout>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: C.textSecondary, flexDirection: 'column', gap: '14px' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: 600 }}>{t("جاري تحميل البيانات...")}</span>
            </div>
                            ...
                            <div className="responsive-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '16px' }}>
                                ...
                                <div>
                                    ...
                                    <input type="date" value={form.date}
                                        ...
                                        style={{ ...IS, textAlign: 'center', background: C.card, height: '42px', borderRadius: '10px', fontSize: '13px', fontFamily: OUTFIT }}
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <CustomSelect
                                            ...
                                            options={suppliers.map(s => ({
                                                value: s.id,
                                                label: s.name,
                                                sub: s.balance < 0 ? `${t('له')}: ${formatNumber(Math.abs(s.balance))} ${cSymbol}` : s.balance > 0 ? `${t('عليه')}: ${formatNumber(s.balance)} ${cSymbol}` : t("رصيد: صفر")
                                            }))}
                                        />
                                    ...
                                    {selectedSupplier && (
                                        <div style={{ marginTop: '8px' }}>
                                            <span style={{ fontSize: '12px', padding: '4px 10px', borderRadius: '20px', fontWeight: 700, background: selectedSupplier.balance < 0 ? `${C.success}15` : `${C.danger}15`, color: selectedSupplier.balance < 0 ? C.success : C.danger, border: `1px solid ${selectedSupplier.balance < 0 ? C.success : C.danger}30` }}>
                                                {selectedSupplier.balance < 0 ? `${t('للمورد طرفنا')} ${formatNumber(Math.abs(selectedSupplier.balance))} ${cSymbol}` : selectedSupplier.balance > 0 ? `${t('على المورد طرفنا')} ${formatNumber(selectedSupplier.balance)} ${cSymbol}` : t("رصيد المورد صفر")}
                                            </span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div style={{ position: 'relative' }}>
                                    <CustomSelect
                                        ...
                                        options={availTreasuries.map(tr => ({
                                            value: tr.id,
                                            label: tr.name,
                                            sub: `${t('رصيد الحالي')}: ${formatNumber(tr.balance)} ${cSymbol}`,
                                        }))}
                                    />
                        ...
                        <div style={SC}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                ...
                                <button type="button" onClick={() => handleSubmit(true)} disabled={submitting || !canSubmit}
                                    ...
                                    <Printer size={18} /> {t("ترحيل وطباعة السند")}
                                </button>
                            </div>
                        </div>
                    </div>
