...
export default function DashboardPage() {
  ...
  const fmt = (n: number) => n.toLocaleString(isRtl ? 'ar-EG-u-nu-latn' : 'en-US');
  ...
  useEffect(() => {
    // Do not set greeting here as it breaks reactivity on language change
    const h = new Date().getHours();
    const tick = () => {
      const d = new Date();
      setTime(d.toLocaleTimeString(isRtl ? 'ar-EG-u-nu-latn' : 'en-US', { hour: '2-digit', minute: '2-digit', hour12: true }));
    };
    tick();
      ...
      <div dir={isRtl ? 'rtl' : 'ltr'} style={{ color: C.textPrimary, fontFamily: CAIRO, padding: '0 0 12px' }}>
        ...
        <div className="dashboard-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', animation: 'fadeUp 0.3s ease both', gap: '20px' }}>
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: 900, margin: 0, color: C.textPrimary, fontFamily: CAIRO, display: 'flex', alignItems: 'center', gap: '8px' }}>
              {t(new Date().getHours() < 12 ? t('صباح الخير') : new Date().getHours() < 17 ? t('مساء الخير') : t('مساء النور'))}، <span style={{ color: C.primary }}>{session?.user?.name || t('مستخدم النظام')}</span>
            </h1>
        ...
        <div className="kpi-grid" style={{
          ...
          {isServices ? (
            ...
            <>
              {hasPage('/sales', 'sales') && <KpiCard label={t("إجمالي المبيعات")} value={fmt(stats.salesTotal)} sub={periodLabel[period]} trend={`↑ ${t('مباشر')}`} trendUp={true} color={C.primary} icon={TrendingUp} delay={0} />}
              {hasPage('/purchases', 'purchases') && <KpiCard label={t("إجمالي المشتريات")} value={fmt(stats.purchasesTotal)} sub={periodLabel[period]} trend={`↓ ${t('مباشر')}`} trendUp={false} color={C.warning} icon={ShoppingCart} delay={60} />}
          ...
          {(hasPage('/sales', 'sales') || hasPage('/purchases', 'purchases')) && (
            <SectionCard title={isServices ? t('إيرادات الخدمات مقابل المصروفات') : t('المبيعات مقابل المشتريات')} icon={BarChart2}
              action={<div style={{ fontSize: '11px', color: C.textSecondary, display: 'flex', gap: '15px' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '8px', height: '8px', borderRadius: '50%', background: C.primary }} />{t(isServices ? t('إيرادات') : t('مبيعات'))}</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '8px', height: '8px', borderRadius: '50%', background: isServices ? C.danger : C.warning }} />{t(isServices ? t('مصروفات') : t('مشتريات'))}</span>
              </div>}>
              <div style={{ padding: '20px 10px 10px', height: '260px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={stats.chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                    ...
                    {hasPage('/sales', 'sales') && <Area type="monotone" dataKey="sales" name={t(isServices ? t('إيرادات') : t('مبيعات'))} stroke={C.primary} strokeWidth={3} fill="url(#gSales)" dot={false} />}
                    {isServices 
                  ...
                  <tbody>
                    {stats.recentInvoices
                      ...
                      .map((inv: any, i: number, arr: any[]) => {
                        const s = statusLabels[inv.type] || { label: inv.type, color: C.textMuted, bg: C.border };
                        return (
