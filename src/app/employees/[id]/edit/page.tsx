            ...
            <DashboardLayout>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: C.textSecondary }}>
                    <div style={{ }}>
                        ...
                        <span style={{ fontSize: '15px', fontWeight: 600 }}>{t('جاري استرجاع بيانات الموظف...')}</span>
                    </div>
                </div>
            </DashboardLayout>
        );
    ...
    return (
        <DashboardLayout>
            <div dir={isRtl ? 'rtl' : 'ltr'} style={{ ...PAGE_BASE, fontFamily: CAIRO }}>
                ...
                <PageHeader 
                    title={t("تعديل الموظف")}
                    subtitle={`${t('تحديث بيانات الموظف:')} ${form.name}`}
                    icon={UsersIcon}
                    backUrl="/employees"
                />
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: '24px', alignItems: 'start' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <FormSection label={t("البيانات الشخصية والتعريفية")} icon={UsersIcon} color="#256af4">
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                    <Field label={t("كود الموظف")}>
                                        <input type="text" readOnly value={form.code} style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', fontWeight: 600, padding: '0 16px', cursor: 'not-allowed', fontFamily: OUTFIT }} />
                                    </Field>
                                    <Field label={t("الاسم الكامل")} required>
                                        <input type="text" required value={form.name} onChange={e => set('name')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder={t("أدخل اسم الموظف")} style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', fontWeight: 700, padding: '0 16px', outline: 'none' }} />
                                    </Field>
                                    ...
                                    <Field label={t("العنوان السكني")}>
                                        <input type="text" value={form.address} onChange={e => set('address')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder={t("المدينة، الحي")} style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', fontWeight: 700, padding: '0 16px' }} />
                                    </Field>
                                </div>
                            </FormSection>
                            ...
                            <FormSection label={t("البيانات الوظيفية والتعاقدية")} icon={Briefcase} color="#8b5cf6">
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                    <Field label={t("القسم الإداري")}>
                                        <CustomSelect value={form.departmentId} onChange={set('departmentId')} icon={Building2} placeholder={t("اختر القسم")} style={{ background: C.inputBg, border: `1px solid ${C.border}` }} options={departments.map(d => ({ value: d.id, label: d.name }))} />
                                    </Field>
                                    {branches.length > 1 && (
                                        <Field label={t("الفرع")}>
                                    ...
                                    <Field label={t("المسمى الوظيفي")}>
                                        <input type="text" value={form.position} onChange={e => set('position')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder={t("المسمى الوظيفي الحالي")} style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', fontWeight: 700, padding: '0 16px' }} />
                                    </Field>
                            ...
                            <FormSection label={t("سلم الرواتب والبدلات")} icon={CreditCard} color="#10b981">
                                <Field label={t("الراتب الأساسي الشهري")} required>
                            ...
                            <FormSection label={t("معلومات التحويل البنكي")} icon={Landmark} color="#f59e0b">
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                    <Field label={t("اسم البنك")}>
                                        <input type="text" value={form.bankName} onChange={e => set('bankName')(e.target.value)} onFocus={focusIn} onBlur={focusOut} style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', fontWeight: 700, padding: '0 16px' }} />
                                    </Field>
                        ...
                        <aside style={{ position: 'sticky', top: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '20px', padding: '24px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                                    ...
                                    <span style={{ fontSize: '13px', fontWeight: 600, color: '#fff' }}>{t('إدارة المرفقات')}</span>
                                </div>
                                <AttachmentUploader
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
                                <button
                                    type="submit"
                                    disabled={isSaving}
                                    style={{ width: '100%', height: '52px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', fontSize: '15px', fontWeight: 600, cursor: isSaving ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: '0 8px 20px rgba(16,185,129,0.3)', transition: '0.2s' }}
                                    onMouseEnter={e => { if (!isSaving) e.currentTarget.style.transform = 'translateY(-2px)'; }}
                                    onMouseLeave={e => { e.currentTarget.style.transform = 'none'; }}
                                >
                                    {isSaving ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />}
                                    {isSaving ? t('جاري الحفظ...') : t('تحديث بيانات الموظف')}
                                </button>
                                <button
                                    type="button"
