...
export default function EditEmployeePage() {
    ...
    useEffect(() => {
        ...
        const fetchContent = async () => {
            setIsLoading(true);
            try {
                const [dRes, eRes, bRes] = await Promise.all([
                ...
                if (bRes.ok) { const bd = await bRes.json(); if (Array.isArray(bd)) setBranches(bd); }
                ...
                if (eRes.ok) {
                    ...
                    setForm({
                        ...
                        insuranceDeduction: String(emp.insuranceDeduction || '0'),
                        taxDeduction: String(emp.taxDeduction || '0'),
            ...
            <DashboardLayout>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: C.textMuted }}>
                    <div style={{ textAlign: 'center' }}>
                        ...
                        <span style={{ fontSize: '15px', fontWeight: 800 }}>جاري استرجاع بيانات الموظف...</span>
                    </div>
                </div>
            </DashboardLayout>
        );
    ...
    return (
        <DashboardLayout>
            <div dir="rtl" style={{ ...PAGE_BASE, fontFamily: CAIRO }}>
                ...
                <PageHeader 
                    title="تعديل الموظف" 
                    subtitle={`تحديث بيانات الموظف: ${form.name}`}
                    icon={UsersIcon}
                    backUrl="/employees"
                />
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: '24px', alignItems: 'start' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <FormSection label="البيانات الشخصية والتعريفية" icon={UsersIcon} color="#3b82f6">
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                    ...
                                    <Field label="الاسم الكامل" required>
                                        <input type="text" required value={form.name} onChange={e => set('name')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder="أدخل اسم الموظف" style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '14px', fontWeight: 700, padding: '0 16px', outline: 'none' }} />
                                    </Field>
                                    <Field label="الرقم القومي">
                                        <input type="text" value={form.nationalId} onChange={e => set('nationalId')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder="14 رقم" maxLength={14} style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '14px', fontWeight: 800, padding: '0 16px', direction: 'ltr', fontFamily: INTER }} />
                                    </Field>
                                    ...
                                    <Field label="العنوان السكني">
                                        <input type="text" value={form.address} onChange={e => set('address')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder="المدينة، الحي" style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '14px', fontWeight: 700, padding: '0 16px' }} />
                                    </Field>
                                </div>
                            </FormSection>
                            ...
                            <FormSection label="البيانات الوظيفية والتعاقدية" icon={Briefcase} color="#8b5cf6">
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                    <Field label="القسم الإداري">
                                        <CustomSelect value={form.departmentId} onChange={set('departmentId')} icon={Building2} placeholder="اختر القسم" style={{ background: C.inputBg, border: `1px solid ${C.border}` }} options={departments.map(d => ({ value: d.id, label: d.name }))} />
                                    </Field>
                                    {branches.length > 1 && (
                                        <Field label="الفرع">
                                    ...
                                    <Field label="المسمى الوظيفي">
                                        <input type="text" value={form.position} onChange={e => set('position')(e.target.value)} onFocus={focusIn} onBlur={focusOut} placeholder="المسمى الوظيفي الحالي" style={{ height: '44px', width: '100%', borderRadius: '12px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '14px', fontWeight: 700, padding: '0 16px' }} />
                                    </Field>
                            ...
                            <FormSection label="سلم الرواتب والبدلات" icon={CreditCard} color="#10b981">
                                <Field label="الراتب الأساسي الشهري" required>
                                    <div style={{ position: 'relative' }}>
                                        ...
                                        />
                                        <span style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', fontFamily: CAIRO, 
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginTop: '14px' }}>
                                    <Field label="بدل سكن">
                                        <div style={{ position: 'relative' }}>
                                            <input type="text" inputMode="decimal" value={formatWithCommas(form.housingAllowance)} onChange={e => set('housingAllowance')(e.target.value.replace(/[^0-9.]/g, ''))} onFocus={focusIn} onBlur={focusOut} placeholder="0.00" style={{ height: '40px', width: '100%', borderRadius: '10px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', paddingLeft: '32px', textAlign: 'center', fontFamily: INTER }} />
                                            <span style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', fontFamily: CAIRO, 
                                            fontWeight: 700, color: C.textMuted, fontSize: '10px' }}>{cSymbol}</span>
                                        </div>
                                    </Field>
                                    <Field label="بدل مواصلات">
                                        <div style={{ position: 'relative' }}>
                                            <input type="text" inputMode="decimal" value={formatWithCommas(form.transportAllowance)} onChange={e => set('transportAllowance')(e.target.value.replace(/[^0-9.]/g, ''))} onFocus={focusIn} onBlur={focusOut} placeholder="0.00" style={{ height: '40px', width: '100%', borderRadius: '10px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', paddingLeft: '32px', textAlign: 'center', fontFamily: INTER }} />
                                            <span style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', fontFamily: CAIRO, 
                                            fontWeight: 700, color: C.textMuted, fontSize: '10px' }}>{cSymbol}</span>
                                        </div>
                                    </Field>
                                    <Field label="بدل غذاء">
                                        <div style={{ position: 'relative' }}>
                                            <input type="text" inputMode="decimal" value={formatWithCommas(form.foodAllowance)} onChange={e => set('foodAllowance')(e.target.value.replace(/[^0-9.]/g, ''))} onFocus={focusIn} onBlur={focusOut} placeholder="0.00" style={{ height: '40px', width: '100%', borderRadius: '10px', border: `1px solid ${C.border}`, background: C.inputBg, color: C.textSecondary, fontSize: '13px', paddingLeft: '32px', textAlign: 'center', fontFamily: INTER }} />
                                            <span style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', fontFamily: CAIRO, 
                                            fontWeight: 700, color: C.textMuted, fontSize: '10px' }}>{cSymbol}</span>
                                        </div>
                                    </Field>
                        ...
                        <aside style={{ position: 'sticky', top: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <div style={{ 
                                ...
                                <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    ...
                                    <div style={{ fontSize: '11px', color: C.textMuted, lineHeight: 1.6, textAlign: 'center' }}>
                                        <Info size={12} style={{ verticalAlign: 'middle', marginLeft: '6px' }} />
                            ...
                            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '20px', padding: '24px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                                    ...
                                    <span style={{ fontSize: '14px', fontWeight: 900, color: '#fff' }}>إدارة المرفقات</span>
                                </div>
                                <AttachmentUploader
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
                                <button
                                    type="submit"
                                    disabled={isSaving}
                                    style={{ width: '100%', height: '52px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', fontSize: '15px', fontWeight: 800, cursor: isSaving ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: '0 8px 20px rgba(16,185,129,0.3)', transition: '0.2s' }}
                                    onMouseEnter={e => { if (!isSaving) e.currentTarget.style.transform = 'translateY(-2px)'; }}
                                    onMouseLeave={e => { e.currentTarget.style.transform = 'none'; }}
                                >
                                    {isSaving ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />}
                                    {isSaving ? 'جاري الحفظ...' : 'تحديث بيانات الموظف'}
                                </button>
                                <button
                                    type="button"
...
function Field({ label, required, children }: any) {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: C.textSecondary, marginRight: '4px' }}>{label}{required && <span style={{ color: THEME.colors.danger }}> *</span>}</label>
            {children}
        </div>
    );
}
