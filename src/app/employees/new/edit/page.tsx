...
function AttachmentUploader({ attachments, onAdd, onRemove }: {
    ...
    const handle = async (file: File) => {
        if (file.size > 10 * 1024 * 1024) { setError(t("الحجم الأقصى 10MB")); return; }
        ...
        try {
            ...
            else throw new Error(data.error || t("فشل الرفع"));
    ...
    return (
        <div>
            ...
            >
                {uploading
                    ? <><Loader2 size={28} color="#6366f1" style={{ animation: 'spin 1s linear infinite' }} /><span style={{ fontSize: '13px', color: '#6366f1', fontWeight: 600 }}>{t("جاري الرفع...")}</span></>
            ...
            <div dir={isRtl ? 'rtl' : 'ltr'} style={{ paddingBottom: '40px' }}>
                ...
                <form onSubmit={submit}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '24px', alignItems: 'start' }}>
                        ...
                        <div>
                            {/* ① شخصية */}
                            <FormSection label={t("البيانات الشخصية")} icon={UsersIcon} color="#256af4" description={t("المعلومات الأساسية وبيانات التواصل والتعريف")}>
                                <Grid2>
                            ...
                            {/* ② وظيفية */}
                            <FormSection label={t("البيانات الوظيفية")} icon={Briefcase} color="#8b5cf6" description={t("تفاصيل التعيين والمسمى الوظيفي والقسم")}>
                                <Grid2>
                            ...
                            {/* ④ بنكية */}
                            <FormSection label={t("بيانات الحساب البنكي")} icon={CreditCard} color="#f59e0b" description={t("معلومات تحويل الراتب والبيانات البنكية")}>
                                <Grid2>
                            ...
                            <div style={{ background: 'linear-gradient(135deg, #1e293b, #0f172a)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '20px', padding: '24px', boxShadow: '0 10px 30px rgba(0,0,0,0.2)' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                        ...
                                        <span style={{ color: '#f1f5f9', fontWeight: 600 }}>{fMoneyJSX(+form.basicSalary || 0)}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
