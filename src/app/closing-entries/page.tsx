    ...
    const handleExecuteFinal = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert(data.error || 'حدث خطأ أثناء التنفيذ');
            }
        } catch (error) {
    ...
    return (
        <DashboardLayout>
            <div dir={isRtl ? 'rtl' : 'ltr'} style={{ ...PAGE_BASE, fontFamily: CAIRO }}>
                ...
                {showConfirm && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>
                        <div style={{ background: '#0e172a', border: `1px solid ${C.border}`, borderRadius: '24px', padding: '32px', maxWidth: '460px', width: '90%', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', position: 'relative' }}>
                            ...
                            <h2 style={{ fontSize: '20px', fontWeight: 900, marginBottom: '12px', color: '#fff' }}>تأكيد تنفيذ الإقفال المحاسبي</h2>
                            <p style={{ fontSize: '13px', color: '#94a3b8', lineHeight: 1.6, marginBottom: '24px' }}>
                                هل أنت متأكد من تصفير حسابات السنة الحالية وترحيل الأرصدة الختامية؟ لا يمكن التراجع عن هذه العملية بمجرد البدء.
                            </p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <button onClick={handleExecuteFinal} style={{ height: '52px', background: 'linear-gradient(135deg, #3b82f6, #2563eb)', border: 'none', borderRadius: '14px', color: '#fff', fontSize: '14px', fontWeight: 800, cursor: 'pointer' }}>تأكيد التنفيذ</button>
                                <button onClick={() => setShowConfirm(false)} style={{ height: '52px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, borderRadius: '14px', color: '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>إلغاء</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {showUndoConfirm && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>
                        <div style={{ background: '#0e172a', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '24px', padding: '32px', maxWidth: '460px', width: '90%', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
                            ...
                            <h2 style={{ fontSize: '20px', fontWeight: 900, marginBottom: '12px', color: '#fff' }}>إلغاء الإقفال المحاسبي</h2>
                            <p style={{ fontSize: '13px', color: '#94a3b8', lineHeight: 1.6, marginBottom: '24px' }}>
                                هل أنت متأكد من رغبتك في التراجع عن الإقفال؟ سيتم حذف قيد الإقفال وإعادة فتح السنة المالية للتعديل.
                            </p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <button onClick={handleUndoFinal} style={{ height: '52px', background: '#ef4444', border: 'none', borderRadius: '14px', color: '#fff', fontSize: '14px', fontWeight: 800, cursor: 'pointer', transition: '0.2s' }}>تأكيد الإلغاء</button>
                                <button onClick={() => setShowUndoConfirm(false)} style={{ height: '52px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', color: '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>تراجع</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {showDraftWarning && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
                        <div style={{ background: '#0e172a', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '24px', padding: '32px', maxWidth: '450px', width: '90%', textAlign: 'center' }}>
                            ...
                            <h2 style={{ fontSize: '18px', fontWeight: 800, marginBottom: '12px', color: '#ef4444' }}>تنبيه: قيود غير مرحلة</h2>
                            <p style={{ fontSize: '13px', color: '#94a3b8', lineHeight: 1.6, marginBottom: '24px' }}>
                                يوجد عدد <strong style={{ color: '#fff' }}>{draftCount}</strong> قيود في حالة "مسودة". يرجى ترحيل كافة القيود قبل إقفال السنة المالية لضمان دقة النتائج.
                            </p>
                            <button onClick={() => setShowDraftWarning(false)} style={{ width: '100%', height: '48px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, borderRadius: '12px', color: '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>حسناً</button>
                        </div>
                    </div>
                )}
                ...
                <PageHeader 
                    title="قيود الإقفال" 
                    subtitle="مرحلة تدوير الأرصدة وإثبات الأرباح الختامية"
                    icon={BookMarked}
                />
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', padding: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '20px', alignItems: 'flex-end' }}>
                                <div style={{ flex: 1 }}>
                                    <label style={{ display: 'block', fontSize: '12px', color: C.textMuted, fontWeight: 800, marginBottom: '8px' }}>السنة المالية المراد إقفالها</label>
                                    <CustomSelect
                                        value={selectedYear}
                                ...
                                <div style={{ flex: 1 }}>
                                    <label style={{ display: 'block', fontSize: '12px', color: C.textMuted, fontWeight: 800, marginBottom: '8px' }}>حساب تجميع الأرباح/الخسائر</label>
                                    <CustomSelect
                                ...
                                {selectedYear && !loading && !alreadyClosed && step !== 'done' && (
                                    ...
                                    >
                                        ...
                                        {executing ? 'جاري التنفيذ...' : 'بدء عملية الإقفال'}
                                    </button>
                                )}
                            </div>
                        </div>
                        ...
                        {!selectedYear ? (
                            <div style={{ textAlign: 'center', padding: '100px 20px', background: 'rgba(255,255,255,0.01)', border: `1px dashed ${C.border}`, borderRadius: '20px', color: C.textMuted }}>
                                ...
                                <h3 style={{ fontSize: '16px', fontWeight: 700 }}>يرجى اختيار السنة المالية للمعاينة</h3>
                                <p style={{ fontSize: '13px', marginTop: '4px' }}>سيتم سحب كافة حسابات الإيرادات والمصروفات لموسم المختارة</p>
                            </div>
                        ) : loading ? (
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px', flexDirection: 'column', gap: '16px', background: C.card, borderRadius: '20px', border: `1px solid ${C.border}` }}>
                                ...
                                <span style={{ fontSize: '14px', color: C.textMuted, fontWeight: 700 }}>جاري تحليل البيانات المحاسبية...</span>
                            </div>
                        ) : alreadyClosed ? (
                            <div style={{ padding: '60px', background: C.card, border: `1px solid ${C.border}`, borderRadius: '24px', textAlign: 'center' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'center', gap: '20px' }}>
                                    ...
                                    >
                                        <RotateCcw size={20} /> إلغاء إقفال السنة
                                    </button>
                                </div>
                            </div>
                        ) : step === 'done' && result ? (
                            <div style={{ padding: '40px', background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.15)', borderRadius: '24px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
                                    ...
                                    <div>
                                        <h2 style={{ fontSize: '20px', fontWeight: 900, color: '#10b981', margin: 0 }}>اكتمل الإقفال بنجاح</h2>
                                        <p style={{ fontSize: '13px', color: C.textMuted, margin: '4px 0 0' }}>تم توليد قيد التصفير وترحيل الأرباح إلى الحساب الختامي.</p>
                                    </div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                                    <StatCard label="إجمالي الإيرادات" value={result.revenueTotal} color="#3b82f6" compact />
                                    <StatCard label="إجمالي المصروفات" value={result.expenseTotal} color="#ef4444" compact />
                                    <StatCard label={result.isProfit ? "صافي أرباح العام" : "صافي خسائر العام"} value={Math.abs(result.netIncome)} color={result.isProfit ? '#10b981' : '#f59e0b'} compact />
                                </div>
                            </div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                                    <StatCard label="رصيد الإيرادات" value={totalRev} color="#3b82f6" icon={ArrowUpRight} />
                                    <StatCard label="رصيد المصروفات" value={totalExp} color="#ef4444" icon={ArrowDownRight} />
                                    <StatCard label={isProfitValue ? "الأرباح المتوقعة" : "الخسائر المتوقعة"} value={Math.abs(netIncomeValue)} color={isProfitValue ? '#10b981' : '#f87171'} icon={TrendingUp} />
                                </div>
                                ...
                                {draftCount > 0 && (
                                    <div style={{ padding: '16px 20px', background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        ...
                                        <span style={{ fontSize: '14px', color: '#ef4444', fontWeight: 800 }}>تنبيه: يوجد {draftCount} قيود "مسودة" غير مرحلة تمنع إتمام الإقفال.</span>
                                    </div>
                                )}

                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                                    <DetailTable title="حسابات الإيرادات" accounts={revenues} color="#3b82f6" />
                                    <DetailTable title="حسابات المصروفات" accounts={expenses} color="#ef4444" />
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <aside style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', padding: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', borderBottom: `1px solid ${C.border}`, paddingBottom: '14px' }}>
                                ...
                                <span style={{ fontSize: '14px', fontWeight: 900, color: '#fff' }}>أرشيف السنوات المرحلة</span>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {years.filter(y => !y.isOpen).length > 0 ? (
                                    ...
                                    <div style={{ textAlign: 'center', padding: '20px 0', opacity: 0.5 }}>
                                        ...
                                        <p style={{ color: C.textMuted, fontSize: '12px' }}>لا توجد سنوات مقفلة بعد</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div style={{ background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.15)', borderRadius: '16px', padding: '20px' }}>
                            <h4 style={{ fontSize: '14px', fontWeight: 900, color: '#3b82f6', margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '8px' }}><Info size={16}/> ملاحظات محاسبية</h4>
                            <p style={{ fontSize: '12px', color: C.textMuted, margin: 0, lineHeight: 1.6 }}>
                                عملية الإقفال تقوم بتصفير حسابات (الإيرادات والمصروفات) ونقل صافي النتيجة إلى حساب الأرباح والخسائر. يرجى مراجعة كافة القيود قبل البدء.
                            </p>
                        </div>
                    </aside>
                </div>
...
function DetailTable({ title, accounts, color }: any) {
    return (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.02)', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '11px', background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: '10px', color: C.textMuted }}>{accounts.length} حساب</span>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <tbody>
                        {accounts.length > 0 ? accounts.map((acc: any, i: number) => (
                            ...
                            <tr>
                                <td colSpan={2} style={{ padding: '40px', textAlign: 'center', color: C.textMuted, fontSize: '12px' }}>لا توجد أرصدة حالياً</td>
                            </tr>
                        )}
                    </tbody>
                </table>
