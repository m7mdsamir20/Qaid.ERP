...
import { FolderKanban, ArrowRight, Loader2, Save, X } from 'lucide-react';
                    ...
                    {loadingLists ? (
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '14px' }}>
                                ...
                                <div>
                                    ...
                                    <select style={{ ...IS, cursor: 'pointer' }} value={form.managerId} onChange={e => setForm({ ...form, managerId: e.target.value })} onFocus={focusIn} onBlur={focusOut}>
                                        <option value="">{t('اختر المهندس / المدير المسؤول...')}</option>
                                        {employees.map((e: any) => (
                            ...
                            <div>
                                ...
                                <textarea rows={2} placeholder={t('أي ملاحظات أو بنود خاصة بالعقد...')} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} style={{ ...IS, height: 'auto', padding: '10px 14px' }} onFocus={focusIn} onBlur={focusOut} />
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '12px', marginTop: '20px' }}>
                                <button disabled={submitting} type="submit" style={{
                                    ...
                                    {submitting ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <><Save size={16} /> {t('إنشاء المشروع الآن')}</>}
                                </button>
