...
export default function DatabaseTab({
    ...
    return (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '24px', padding: '32px', boxShadow: '0 10px 40px -15px rgba(0,0,0,0.5)', minHeight: '600px' }}>
            <TabHeader
                title={t("قاعدة البيانات والنسخ الاحتياطي")}
                sub={t("إدارة بياناتك، تحميل نسخ احتياطية، أو استعادة النظام")}
                hideEditBtn
                t={t}
            />
                ...
                <div className="mobile-setting-row" style={{ display: 'flex', alignItems: 'center', borderBottom: `1px solid ${C.border}` }}>
                    <div className="mobile-setting-label" style={{ width: '220px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px', padding: '16px 20px', borderInlineStart: `1px solid ${C.border}`, background: 'rgba(255,255,255,0.01)' }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '10px', color: C.textSecondary, marginTop: '2px', fontFamily: CAIRO }}>{t('مثالي للاستعادة الكاملة')}</div>
                        </div>
                    </div>
                        ...
                        <button className="action-btn"
                            onClick={async () => {
                                try {
                                    ...
                                    if (!res.ok) { alert(t('فشل التحميل')); return; }
                ...
                <div className="mobile-setting-row" style={{ display: 'flex', alignItems: 'center', borderBottom: `1px solid ${C.border}` }}>
                    <div className="mobile-setting-label" style={{ width: '220px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px', padding: '16px 20px', borderInlineStart: `1px solid ${C.border}`, background: 'rgba(255,255,255,0.01)' }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '10px', color: C.textSecondary, marginTop: '2px', fontFamily: CAIRO }}>{t('للمراجعة والطباعة')}</div>
                        </div>
                    </div>
                    <div className="mobile-setting-value" style={{ flex: 1, padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                        <span style={{ fontSize: '12px', color: C.textSecondary, fontFamily: CAIRO }}>
                ...
                <div className="mobile-setting-row" style={{ display: 'flex', alignItems: 'center' }}>
                    <div className="mobile-setting-label" style={{ width: '220px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px', padding: '16px 20px', borderInlineStart: `1px solid ${C.border}`, background: 'rgba(255,255,255,0.01)' }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '10px', color: C.textSecondary, marginTop: '2px', fontFamily: CAIRO }}>{t('من ملف JSON فقط')}</div>
                        </div>
                    </div>
                    <div className="mobile-setting-value" style={{ flex: 1, padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                        <span style={{ fontSize: '12px', color: C.textSecondary, fontFamily: CAIRO }}>
                                    ...
                                    try {
                                        ...
                                        if (res.ok) {
                                            alert(
                                                ...
                                                `${t('فواتير')}: ${d.stats.invoices}\n` +
                                                `${t('موظفين')}: ${d.stats.employees}`
                                            );
                                        } else {
                                            alert('❌ ' + d.error);
                                        }
                                    } catch { alert('❌ ' + t('ملف غير صالح')); }
                                    e.target.value = '';
                                }} />
                        </label>
                    </div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 20px -10px rgba(0,0,0,0.3)' }}>
                {[
                    { id: 'customers', label: t('العملاء'), icon: <Users size={15} />, color: C.primary, desc: t('استيراد قائمة العملاء وأرصدتهم الافتتاحية') },
                ...
                <div onClick={() => !importLoading && setShowImportModal(false)} style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,8,23,0.85)', backdropFilter: 'blur(8px)' }}>
                    <div onClick={e => e.stopPropagation()} dir="rtl" style={{ width: '100%', maxWidth: importStep === 2 && importType !== 'items' ? '820px' : importStep === 2 && importType === 'items' ? '780px' : '620px', background: '#0f172a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', transition: 'max-width 0.3s ease' }}>
                        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    color: C.textSecondary, cursor: importLoading ? 'not-allowed' : 'pointer',
                        ...
                        <div style={{ padding: '24px' }}>
                            {importStep === 1 && (
                                <div style={{ padding: '40px 20px', border: '2px dashed rgba(37, 106, 244,0.2)', borderRadius: '16px', background: 'rgba(37, 106, 244,0.02)', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                            ...
                            {importStep === 2 && (
                                <div>
                                    <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '12px', padding: '14px 18px', color: '#10b981', display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                                        <Check size={18} />
                                        <span style={{ fontWeight: 700, fontFamily: CAIRO, fontSize: '13px' }}>{t('تم العثور على')} {importData.length} {t('سجل — راجع البيانات قبل الحفظ')}</span>
                                    </div>
                                    ...
                                    {importType === 'items' && (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', padding: '14px 18px', background: 'rgba(37, 106, 244,0.05)', border: `1px solid ${C.primary}20`, borderRadius: '12px' }}>
                                            ...
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: '11px', fontWeight: 700, color: C.textSecondary, fontFamily: CAIRO, marginBottom: '6px' }}>{t('المخزن المستهدف')}</div>
                                                {warehouses.length === 0 ? (
                                                    <div style={{ fontSize: '12px', color: '#f59e0b', fontFamily: CAIRO, fontWeight: 700 }}>⚠️ {t('لا يوجد مخازن — أضف مخزناً أولاً من صفحة المخازن')}</div>
                                                ) : (
                                                    <CustomSelect
                                    ...
                                    <div style={{ maxHeight: '320px', overflowY: 'auto', borderRadius: '12px', border: `1px solid ${C.border}`, background: 'rgba(0,0,0,0.2)' }} className="custom-scrollbar">
                                                ...
                                                {importData.slice(0, 20).map((row: any, i: number) => {
                                                    ...
                                                    return (
                                                        <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}
                                                            ...
                                                            <td style={{ padding: '11px 14px',  color: C.textSecondary, fontSize: '12px' }}>
                                                                {importType === 'items'
                                                            ...
                                                            {importType === 'items' ? (
                                                                <>
                                                                    <td style={{ padding: '11px 14px',  fontWeight: 600, color: C.textSecondary, fontFamily: 'monospace' }}>
                                                                        {formatNumber(findV([t("كمية افتتاحية"), t("كمية حالية"), t("الكمية"), t("كمية"), 'stock', 'qty', 'quantity']))}
                                                                    </td>
                                                                    <td style={{ padding: '11px 14px',  fontWeight: 600, color: C.primary, fontFamily: 'monospace' }}>
                                                                        {formatNumber(findV([t("تكلفة"), 'cost']))}
                                                                    </td>
                                                                    <td style={{ padding: '11px 14px',  fontWeight: 600, color: '#f59e0b', fontFamily: 'monospace', background: 'rgba(245,158,11,0.03)' }}>
                                                                        {formatNumber(findV([t("كمية افتتاحية"), t("كمية حالية"), t("الكمية"), t("كمية"), 'stock', 'qty', 'quantity']) * findV([t("تكلفة"), 'cost']))}
                                                                    </td>
                                                                </>
                                                            ) : (
                                                                <>
                                        ...
                                        {importData.length > 20 && (
                                            <div style={{ padding: '12px 14px', color: C.textSecondary, fontSize: '12px', fontFamily: CAIRO, borderTop: `1px solid ${C.border}`, background: 'rgba(255,255,255,0.01)' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
                                        ...
                                        <button onClick={() => { setImportStep(1); setImportData([]); }} style={{ height: '46px', padding: '0 24px', borderRadius: '12px', border: `1px solid ${C.border}`, background: 'transparent', color: C.textSecondary, fontWeight: 700, cursor: 'pointer', fontFamily: CAIRO }}>{t('إلغاء')}</button>
                                    </div>
                                </div>
                            )}
                            ...
                            {importStep === 3 && (
                                <div style={{ padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                                    ...
                                    <p style={{ margin: '0 0 24px', color: C.textSecondary, fontFamily: CAIRO }}>
