...
export default function FinancialYearTab({ financialYears, isSaving, saveSettings, setConfirmDelete }: FinancialYearTabProps) {
    ...
    return (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '24px', padding: '32px', boxShadow: '0 10px 40px -15px rgba(0,0,0,0.5)', minHeight: '600px' }}>
            <div>
                <TabHeader title="السنة المالية" sub="إدارة الفترات المالية وتسلسلها" hideEditBtn />
                ...
                {!activeFY && closedYears.length === 0 && (
                    <div style={{ background: 'rgba(59,130,246,0.04)', border: `1px solid ${C.primary}30`, borderRadius: '16px', padding: '32px', textAlign: 'center' }}>
                        ...
                        <h3 style={{ margin: '0 0 8px', fontSize: '16px', fontWeight: 900, color: C.textPrimary, fontFamily: CAIRO }}>تأسيس الدورة المحاسبية</h3>
                        ...
                        <button onClick={() => {
                            ...
                            if (!s || !e) { alert('حدد التواريخ'); return; }
                            if (new Date(e) <= new Date(s)) { alert('النهاية يجب أن تكون بعد البداية'); return; }
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '20px', padding: '32px', display: 'flex', alignItems: 'center', gap: '40px', boxShadow: '0 4px 20px -10px rgba(0,0,0,0.3)' }}>
                            ...
                            <div style={{ position: 'relative', width: '120px', height: '120px' }}>
                                ...
                                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                                    ...
                                    <span style={{ fontSize: '10px', color: C.textMuted, fontWeight: 800, fontFamily: CAIRO, textTransform: 'uppercase' }}>مكتمل</span>
                                </div>
                            </div>
                            ...
                            <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                                    ...
                                    <span style={{ background: 'rgba(16,185,129,0.1)', color: '#10b981', padding: '4px 12px', borderRadius: '20px', fontSize: '10px', fontWeight: 900, border: '1px solid rgba(16,185,129,0.2)', fontFamily: CAIRO }}>دورة نشطة</span>
                                </div>

                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                                    {[
                                        { label: 'تاريخ البداية', value: fmt(activeFY.startDate), icon: Calendar },
                                        { label: 'تاريخ الاستحقاق', value: fmt(activeFY.endDate), icon: CalendarCheck },
                                        { label: 'الأيام المتبقية', value: `${remaining} يوم`, icon: Clock, highlight: true }
                                    ].map((item, i) => (
                        ...
                        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '20px', padding: '24px', boxShadow: '0 4px 20px -10px rgba(0,0,0,0.3)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                                <div>
                                    <h4 style={{ margin: '0 0 4px', fontSize: '15px', fontWeight: 900, color: C.textPrimary, fontFamily: CAIRO }}>إغلاق السنة المالية</h4>
                                    <p style={{ margin: 0, fontSize: '12px', color: C.textMuted, fontFamily: CAIRO, lineHeight: 1.5 }}>عند إغلاق السنة، سيتم ترحيل الأرصدة الافتتاحية وفتح فترة جديدة تبدأ من <span style={{ color: C.primary, fontWeight: 900 }}>{fmt(nextStart)}</span></p>
                                </div>
                                <div style={{ background: `${C.danger}15`, color: C.danger, padding: '6px 14px', borderRadius: '10px', fontSize: '11px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px', border: `1px solid ${C.danger}30`, fontFamily: CAIRO }}>
                                    <AlertCircle size={14} /> إجراء حساس
                                </div>
                            </div>

                            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '20px' }}>
                                <div style={{ flex: 1 }}>
                                    <label style={{ display: 'block', fontSize: '12px', color: C.textSecondary, marginBottom: '8px', fontFamily: CAIRO, fontWeight: 800 }}>تحديد تاريخ نهاية السنة الجديدة</label>
                ...
                {closedYears.length > 0 && (
                    <div style={{ marginTop: '32px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                            ...
                            <h3 style={{ margin: 0, fontSize: '12px', fontWeight: 900, color: C.textSecondary, fontFamily: CAIRO, textTransform: 'uppercase', letterSpacing: '0.5px' }}>الأرشيف الضريبي (الفترات المقفلة)</h3>
                        </div>
                                ...
                                <tbody>
                                    {closedYears.map((fy, i) => (
                                        <tr key={fy.id} style={{ borderBottom: i < closedYears.length - 1 ? `1px solid ${C.border}` : 'none', transition: 'background 0.2s' }}
                                            ...
                                            <td style={{ padding: '14px 20px', fontSize: '12px', color: C.textMuted, fontFamily: CAIRO }}>{calcDays(fy.startDate, fy.endDate)} يوم</td>
