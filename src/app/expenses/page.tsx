...
export default function ExpensesPage() {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [eRes, aRes, tRes, cRes] = await Promise.all([
                fetch('/api/expenses'),
                fetch('/api/accounts'),
            ...
            setAccounts(aData.filter((a: any) => a.type === 'expense' && (a.accountCategory === 'detail' || !a.isParent)));
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setShowForm(false);
                setForm({ date: new Date().toISOString().split('T')[0], amount: '', accountId: '', treasuryId: '', costCenterId: '', notes: '' });
                fetchData();
            } else {
                    ...
                    <table style={TABLE_STYLE.table}>
                        <thead>
                            <tr style={TABLE_STYLE.thead}>
                                ...
                                <th style={{ ...TABLE_STYLE.th(false, true), textAlign: 'center' }}>{t('المبلغ')}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                ...
                                return (
                                    <tr key={e.id} style={TABLE_STYLE.row(idx === paginated.length - 1)}
                                        ...
                                        <td style={{ ...TABLE_STYLE.td(true, true), color: C.textSecondary, fontSize: '12px', textAlign: 'center' }}>{new Date(e.date).toLocaleDateString('en-GB')}</td>
                                        ...
                                        <td style={TABLE_STYLE.td(false)}>
                                            <div style={{ fontWeight: 600, color: C.textPrimary }}>{debitLine?.account?.name}</div>
                                        ...
                                        <td style={{...TABLE_STYLE.td(false, true)}}>
                                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: C.textPrimary, fontWeight: 700, fontSize: '13px' }}>
                                                    ...
                                                    {e.sourceName || creditLine?.account?.name || '—'}
                                                </div>
                                                <div style={{ 
                                        ...
                                        <td style={{...TABLE_STYLE.td(false, true), textAlign: 'center'}}>
                                            <span style={{ fontSize: '13px', fontWeight: 600, color: C.danger, fontFamily: OUTFIT }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', padding: '10px 0 0 0' }}>
                        <div>
                            ...
                            <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                                style={{ ...IS, direction: 'ltr', height: '42px' }} onFocus={focusIn} onBlur={focusOut} />
                        </div>
                            ...
                            >
                                ...
                                <input 
                                    ...
                                    style={{ 
                                        ...
                                        fontWeight: 600, color: C.danger, height: '100%', 
