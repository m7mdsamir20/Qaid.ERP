...
export const GET = withProtection(async (request: NextRequest, session, body, context) => {
    try {
        ...
        if (!plan) {
            if (wantsHtml) return new NextResponse('<h2>جدول الأقساط غير موجود</h2>', { status: 404, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
            return NextResponse.json({ error: 'جدول الأقساط غير موجود' }, { status: 404 });
        }

        if (wantsHtml) {
            const { generateInstallmentPlanHTML } = await import('@/lib/printInvoices');
            const html = generateInstallmentPlanHTML(plan, company as any, { noAutoPrint: noPrint });
            return new NextResponse(html, {
                headers: { 'Content-Type': 'text/html; charset=utf-8' },
            });
        }

        return NextResponse.json({ plan, company });
    } catch (error: any) {
